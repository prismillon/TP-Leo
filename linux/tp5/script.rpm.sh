...
finalize_yum_repo ()
{
  ...
  yum install -y yum-utils --disablerepo='pufferpanel_pufferpanel'
  ...
  echo "Generating yum cache for pufferpanel_pufferpanel..."
  yum -q makecache -y --disablerepo='*' --enablerepo='pufferpanel_pufferpanel'

  echo "Generating yum cache for pufferpanel_pufferpanel-source..."
  yum -q makecache -y --disablerepo='*' --enablerepo='pufferpanel_pufferpanel-source'
}

finalize_zypper_repo ()
{
  zypper --gpg-auto-import-keys refresh pufferpanel_pufferpanel
  zypper --gpg-auto-import-keys refresh pufferpanel_pufferpanel-source
}
...
main ()
{
  ...
  yum_repo_config_url="https://packagecloud.io/install/repositories/pufferpanel/pufferpanel/config_file.repo?os=${os}&dist=8&source=script"

  if [ "${os}" = "sles" ] || [ "${os}" = "opensuse" ]; then
    yum_repo_path=/etc/zypp/repos.d/pufferpanel_pufferpanel.repo
  else
    yum_repo_path=/etc/yum.repos.d/pufferpanel_pufferpanel.repo
  fi
